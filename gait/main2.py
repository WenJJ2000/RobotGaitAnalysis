...
import numpy as np
import cv2
from gait.data_processing import landmark_to_points
from gait_analysis import (
    butter_lowpass_filter,
    butterworth_filter_landmarks,
    get_left_joint_angles,
)
...
shoulder, hip, knee, ankle, heel, foot = [], [], [], [], []
frame_count = 0
...
with mp_pose.Pose(
    ...
    while raspi_cam.isOpened():
        ...
        if results.pose_world_landmarks:
            # append relevant points to the list
            # left shoulder values
            shoulder.append(
                landmark_to_points(
                    results.pose_world_landmarks.landmark[
                        get_landmark_index("left_shoulder")
                    ]
                )
            )
            # left hip values
            ...
            foot.append(
                landmark_to_points(
                    results.pose_world_landmarks.landmark[
                        get_landmark_index("left_foot_index")
                    ]
                )
            )
            # left heel values
            heel.append(
                landmark_to_points(
                    results.pose_world_landmarks.landmark[
                        get_landmark_index("left_heel")
                    ]
                )
            )
            data = [shoulder, hip, knee, ankle, foot, heel]
            if frame_count > 9:
                data = data.map(lambda x: butterworth_filter_landmarks(x))

            hip_angle, knee_angle, ankle_angle = get_left_joint_angles()

            hip_vals.append(hip_angle)
            knee_vals.append(knee_angle)
            ankle_vals.append(ankle_angle)
